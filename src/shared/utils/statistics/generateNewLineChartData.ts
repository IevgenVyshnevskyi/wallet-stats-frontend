...
export const generateNewLineChartData = (categoryTransactions: Transactions): {
  ...
  Object.entries(categoryTransactions)?.forEach(([dateStr, transactionsArr]) => {
    ...
    totalAmount = transactionsArr.reduce((acc, transaction) => {
      return acc + parseInt(transaction.amount_of_funds);
    }, 0);
  });
