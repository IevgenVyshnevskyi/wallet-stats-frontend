...
const DataEntryPage: React.FC = () => {
  ...
  const { isEntryDataSuccess, entryDataError, isLoading } = useAppSelector(
    (state) => state.wallet
  );

  const { user, getDetailsError, isRegistered } = useAppSelector(
    (state) => state.user
  );
      ...
      <Box
        ...
        <Box m="auto 0">
          ...
          <Typography
            ...
            color={COLORS.ALMOST_BLACK_FOR_TEXT}
            textAlign="center">
            Ви можете внести актуальні дані по <br /> вашому готівковому та
            картковому <br /> рахунках.
          </Typography>
          <Form
            ...
            {getDetailsError && (
              <Typography as="p" textAlight="center">
                {JSON.stringify(getDetailsError)}
              </Typography>
            )}

            {entryDataError && (
              <Typography as="p" textAlight="center">
                {JSON.stringify(entryDataError)}
              </Typography>
            )}
